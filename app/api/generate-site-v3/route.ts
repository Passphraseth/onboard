 ...
 *
 * This is the new generation endpoint that creates truly unique websites
 * based on comprehensive brand profile data.
 *
 * Key differences from v2:
 * - Uses BrandProfile from extraction layer
 * - No hardcoded templates or layouts
 * - Every site is unique based on actual data
 * - AI prompt is structured around the data, not templates
 */
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const extractionInput: ExtractionInput = {
      ...
      existingWebsite: body.website,
      instagramUsername: body.instagram?.replace('@', ''),
      facebookUrl: body.facebook,
